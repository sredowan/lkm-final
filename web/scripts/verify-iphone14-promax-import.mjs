...
const CATEGORY_NAME = 'iPhone 14 Pro Max';
...
async function verify() {
    ...
    try {
        ...
        if (fs.existsSync(uploadDir)) {
            ...
            const iphone14Files = files.filter(f => f.includes('iphone-14-') || f.includes('pro-max') || f.includes('extra'));
            console.log(`\n✔ Found ~${iphone14Files.length} images in ${uploadDir} that might be related to this import.`);

            if (iphone14Files.length < products.length) {
                console.log(`  ⚠️ WARNING: Upload directory has fewer files (${iphone14Files.length}) than products (${products.length}). Some images might have failed to download or have different naming.`);
            }
        } else {
