...
// --- Configuration ---
const DATA_FILE = path.join(__dirname, '../../../../Downloads/scraped_data_ac01574e-e26a-41d4-8e41-1b921497d8bf.json');
const PARENT_CATEGORY_NAME = 'iPhone Accessories';
const SUB_CATEGORY_NAME = 'iPhone 14';
...
async function verify() {
    ...
    try {
        console.log(`Verifying imports for category: ${SUB_CATEGORY_NAME}`);

        // 1. Check Category
        const [categories] = await connection.execute('SELECT id, name FROM categories WHERE name = ?', [SUB_CATEGORY_NAME]);
        if (categories.length === 0) {
            console.error(`Error: Category '${SUB_CATEGORY_NAME}' not found.`);
            return;
        }
        const categoryId = categories[0].id;
        console.log(`✔ Category '${SUB_CATEGORY_NAME}' found with ID ${categoryId}`);
        ...
        if (fs.existsSync(uploadDir)) {
            ...
            const thisCatFiles = files.filter(f => f.includes('iphone-14') && !f.includes('plus') && !f.includes('pro'));
            console.log(`\n✔ Found roughly ~${thisCatFiles.length} images in ${uploadDir} related to imports.`);
