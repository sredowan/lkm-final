...
const JSON_FILE_PATH = 'C:/Users/ADMIN/Downloads/scraped_data_465ead65-32b1-4e98-b991-7d9703e9d668.json';
const PARENT_CATEGORY_NAME = 'iPhone Accessories';
const SUB_CATEGORY_NAME = 'iPhone 16 Pro';
...
function extractTag(title) {
    ...
    if (lowerTitle.includes('screen protector') || lowerTitle.includes('glass')) return 'screen protector';
    if (lowerTitle.includes('lens')) return 'lens protector';
    if (lowerTitle.includes('cable') || lowerTitle.includes('charger')) return 'charger';
    if (lowerTitle.includes('mount') || lowerTitle.includes('holder')) return 'mount';
    return 'accessory';
}
            ...
            try {
                ...
                if (existing.length > 0) {
                    ...
                    await connection.execute(`
                        UPDATE products 
                        SET price = ?, compare_price = ?, stock = ?, description = ?, category_id = ?, tags = ?
                        WHERE id = ?
                    `, [price, comparePrice, stock, description, subCategoryId, tag, productId]);
                    console.log(`Updated: ${name}`);
                } else {
                    // Insert
                    const [res] = await connection.execute(`
                        INSERT INTO products (name, slug, sku, description, price, compare_price, stock, category_id, tags, is_active, is_featured)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 0)
                    `, [name, slug, item['sku'] || null, description, price, comparePrice, stock, subCategoryId, tag]);
                ...
                // Process Main Image
                if (mainImageUrl) {
                    // Try to get extension from URL or fallback
