...
const CATEGORY_NAME = 'iPhone 15 Pro';
...
async function verify() {
    ...
    try {
        ...
        if (fs.existsSync(uploadDir)) {
            ...
            const iphone15ProFiles = files.filter(f => f.includes('15-pro') || f.includes('pro-15') || f.includes('extra'));
            console.log(`\n✔ Found ~${iphone15ProFiles.length} images in ${uploadDir} that might be related to this import.`);

            if (iphone15ProFiles.length < products.length) {
                console.log(`  ⚠️ WARNING: Upload directory has fewer files (${iphone15ProFiles.length}) than products (${products.length}). Some images might have failed to download or have different naming.`);
            }
        } else {
