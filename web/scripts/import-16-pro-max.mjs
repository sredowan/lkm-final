...
const JSON_PATH = 'c:\\Users\\ADMIN\\OneDrive\\Documents\\PERSONAL\\DEVELOPMENTS\\LKM\\JSON IMPORT\\16promax.json';
const PARENT_CATEGORY = 'iPhone Accessories';
const CHILD_CATEGORY = '16 Pro Max';
...
async function run() {
    console.log('Starting iPhone 16 Pro Max accessories import (V2)...');
        ...
        for (const item of products) {
            ...
            if (existing.length > 0) {
                ...
                const [result] = await connection.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [title, slug, sku, description, price, comparePrice, categoryId, STOCK_QUANTITY, tag, true]
                );
                productId = result.insertId;
                console.log(`Added new product [${count}/${products.length}]: ${title}`);
            }

            // 3. Sync with global_tags
            try {
                await connection.execute('INSERT INTO global_tags (name) VALUES (?) ON DUPLICATE KEY UPDATE name = name', [tag]);
            } catch (e) { /* ignore */ }
            ...
            for (const imgData of imagesToDownload) {
                ...
                const filename = `${slug}-${sortOrder}${ext}`;
