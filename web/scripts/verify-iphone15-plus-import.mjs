...
const CATEGORY_NAME = 'iPhone 15 Plus';
...
async function verify() {
    ...
    try {
        ...
        if (fs.existsSync(uploadDir)) {
            ...
            const iphone15PlusFiles = files.filter(f => f.includes('15-plus') || f.includes('plus-15') || f.includes('extra'));
            console.log(`\n✔ Found ~${iphone15PlusFiles.length} images in ${uploadDir} that might be related to this import.`);

            if (iphone15PlusFiles.length < products.length) {
                console.log(`  ⚠️ WARNING: Upload directory has fewer files (${iphone15PlusFiles.length}) than products (${products.length}). Some images might have failed to download or have different naming.`);
            }
        } else {
